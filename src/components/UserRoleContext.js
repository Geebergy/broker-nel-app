...
export const UserProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async (userId) => {
      try {
        const response = await fetch(`https://broker-nel-app-oial.onrender.com/api/userDetail/${userId}`);
        if (!response.ok) {
    ...
    const checkUserExists = async (userId) => {
      try {
        const response = await fetch(`https://broker-nel-app-oial.onrender.com/api/userExists/${userId}`);
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
        const data = await response.json();
