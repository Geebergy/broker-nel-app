    ...
    try {
      if(selectedStarterValue.trim() !== '') {
        const data = {
          ...
          ipn_callback_url: 'https://broker-nel-app-oial.onrender.com/api/crypto-callback',
          order_id: getUniqueId,
          order_description: 'Crypto Deposit'
        };
      
        const response = await axios.post('https://broker-nel-app-oial.onrender.com/api/payment', { data });
...
const saveBtcTransactionData = async (transactionReference, email, amount, userID, status, paymentID) => {
  ...
  if(user){
    ...
    await fetch(`https://broker-nel-app-oial.onrender.com/api/createTransactions`,
   {
    method: 'POST',
    headers: {
...
const saveTempCryptoData = async (userID, payment_id, payment_status, pay_address, price_amount, order_description) => {
  ...
  if(user){
    ...
    await fetch(`https://broker-nel-app-oial.onrender.com/api/saveCryptoPayments`,
   {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
const debitUser = (ticketFee) =>{
  ...
  if (userBalance >= ticketFee && ticketFee !== null){
      const addParticipant = async () => {
          try {
              await fetch('https://broker-nel-app-oial.onrender.com/api/addParticipant', {
                  method: 'POST',
                  headers: {
                      'Content-Type': 'application/json',
                  },
