...
const PaymentModal = () => {
  ...
  useEffect(() => {
    // Fetch wallet addresses from the backend when the component loads
    fetch(`https://broker-nel-app-oial.onrender.com/api/fetchWallets`)
      .then(response => response.json())
      .then(data => {
        // Ensure the data is an array, otherwise set an empty array
        if (Array.isArray(data)) {
          setWalletAddresses(data);
        } else {
          setWalletAddresses([]); // Fallback if the response is not an array
        }
      })
      .catch(err => {
        console.error('Failed to fetch wallet addresses:', err);
        setWalletAddresses([]); // Fallback in case of an error
      });
  }, []); // Dependency array will re-fetch data if agentCode changes
  ...
  const saveBtcTransactionData = async (email, amount, userID, status, paymentID, username) => {
    ...
    try {
      const response = await fetch(`https://broker-nel-app-oial.onrender.com/api/createTransactions`, {
        method: 'POST',
        headers: {
  ...
  const saveTempCryptoData = async (userID, payment_status, pay_address, price_amount, paymentID, username) => {
    ...
    try {
      const response = await fetch(`https://broker-nel-app-oial.onrender.com/api/saveCryptoPayments`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDebitUser = async (amount) => {
    ...
    if(user_balance && amount && user_balance >= amount){
      ...
      try {
        const response = await axios.post('https://broker-nel-app-oial.onrender.com/api/debitUser', {
          userId: userID,
  ...
  return (
    <div className="payment-container" style={{ background: '#13151b', overflowY: 'auto', padding: '20px' }}>
      <ToastContainer />
  ...
  <div className='container-custom'>
    ...
    <div className='d-flex justify-content-center mb-5 fund-wallet-box'>
        <div className="payment-modal container-fluid text-secondary" style={{ width: '100%', maxWidth: '300px', backgroundColor: '#1F222D', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.2)', borderRadius: '8px' }}>
          <div className="mt-3">
            <h4 className='mb-3 text-center'>Fund your Wallet</h4>
            ...
            {showAddress && fundAddress ? (
              ...
              <>
                <input
                  ...
                  className='border border-secondary rounded p-2 w-100'
                  value={fundAmount}
            ...
            <div className='mt-3 text-center'>
              <a href="mailto:nexusfxinvestmentblog@gmail.com">Send us an email</a> to deposit with your bank.
            </div>
          </div>
        </div>
      </div>
