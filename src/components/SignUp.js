...
const handleCountryChange = (selectedOption) => {
  ...
  const createUserInBackend = async (payLoad) => {
    return await fetchAPI('https://broker-nel-app-oial.onrender.com/api/createUser', {
      method: 'POST',
      headers: {
  ...
  const updateReferrerData = async (referrerID) => {
    ...
    return await fetchAPI('https://broker-nel-app-oial.onrender.com/api/updateInfo', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const checkReferralCode = async (referralCode) => {
    const data = await fetchAPI(`https://broker-nel-app-oial.onrender.com/api/checkUserReferral/${referralCode}`);
    return data;
  };

  const checkAgentCode = async (agentCode) => {
    const data = await fetchAPI(`https://broker-nel-app-oial.onrender.com/api/checkAgentCode/${agentCode}`);
    return data;
  };
