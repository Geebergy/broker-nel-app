...
const BtcTaskList = () => { // Rename the function to start with an uppercase letter
  ...
  //   try {
  //     const response = await axios.get(`https://broker-nel-app-oial.onrender.com/api/getBtcDeposits/${agentID}`);
  ...
  const fetchPendingBtcDeposits = async () => {
    try {
      const response = await axios.get(`https://broker-nel-app-oial.onrender.com/api/getBtcDeposits`);
      const transactions = response.data; // axios automatically parses JSON responses
      setBtcTx(transactions);
    } catch (error) {
      console.error('Error fetching BTC deposits:', error.message);
  ...
  const changePaymentStatus = async (transactionId, newStatus, userId, amount) => {
    try {
      const response = await fetch(`https://broker-nel-app-oial.onrender.com/api/updatePaymentStatusAndDelete/${transactionId}`, {
        method: 'PUT',
        headers: {
