...
const initializeTooltip = (element) => {
  ...
  return (
    ...
    <ToastContainer />
      ...
      <nav style={{ borderRadius: '30px'}} className={`navbar ${userData.role === 'checker' ? 'site-nav-checker' : 'site-nav'} ${location.pathname === '/login' || location.pathname === '/signup' || location.pathname === '/admin'  ? 'd-none' : ''} navbar-dark bg-light`}>
