...
import WalletManager from './UpdateWallet';
...
const UserManagement = () => {
  ...
  //     const response = await axios.get(`https://broker-nel-app-oial.onrender.com/api/users?agentID=${agentID}`);
  ...
  const fetchUsers = async () => {
    ...
    try {
      // Include the agentID as a query parameter in the API request
      const response = await axios.get(`https://broker-nel-app-oial.onrender.com/api/users`);
      setUsers(response.data); // Assuming response.data is an array of users
    } catch (error) {
      console.error('Error fetching users:', error);
    } finally {
