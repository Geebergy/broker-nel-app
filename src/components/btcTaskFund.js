...
const BtcFundingList = () => { // Rename the function to start with an uppercase letter
  ...
  const fetchPendingBtcFundings = async () => {
    try {
      const response = await fetch('https://broker-nel-app-oial.onrender.com/api/getBtcFundings');
  ...
  const updateUserBalance = async (transactionId, newStatus, userId, price_amount) => {
    try {
      const response = await fetch(`https://broker-nel-app-oial.onrender.com/api/updateUserBalance/${transactionId}`, {
        method: 'PUT',
        headers: {
